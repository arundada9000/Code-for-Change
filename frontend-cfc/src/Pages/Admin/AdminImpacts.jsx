...
const SelectField = React.memo(({ label, value, onChange, options, required = false }) => (
  <div className="space-y-1.5">
    <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-4">{label}</label>
    <select
      required={required}
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          <h1 className="text-4xl font-black text-primary tracking-tightests mb-2">Our <span className="text-blue-600">Impacts</span></h1>
        ...
        {hasPermission('impact_create') && (
          ...
          >
            <FaPlus className="group-hover:rotate-90 transition-transform" /> CREATE IMPACT
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Impacts</p>
          ...
          <table className="min-w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredImpacts.map((impact) => (
                <tr key={impact._id} className="hover:bg-slate-50/50 transition-all group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-2xl overflow-hidden shadow-sm border border-slate-200">
                        <img src={impact.image} className="w-full h-full object-cover" alt={impact.title} />
                      </div>
                      <div>
                        <div className="font-black text-gray-900 leading-none mb-1">{impact.title}</div>
                        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
                          <FaMapMarkerAlt className="text-emerald-500" size={8} /> {impact.province || "National"} • {impact.location || "Online"}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="space-y-1">
                      <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${impact.category === 'Current' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-white'}`}>
                        {impact.category}
                      </span>
                      <div className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">{impact.dates}</div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6">
                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${impact.isLarge ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-400'}`}>
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      {hasPermission('impact_update') && (
                        <button onClick={() => handleEdit(impact)} className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 hover:bg-blue-50 hover:text-blue-600 transition-all active:scale-90">
                          <FaEdit />
                        </button>
                      )}
                      {hasPermission('impact_delete') && (
                        <button onClick={() => { setImpactToDelete(impact); setDeleteModalOpen(true); }} className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 hover:bg-rose-50 hover:text-rose-600 transition-all active:scale-90">
                          <FaTrash />
                        </button>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-4xl max-h-[90vh] overflow-hidden relative shadow-2xl animate-modal-in flex flex-col">
            <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
              <div>
                <h2 className="text-3xl font-black text-primary tracking-tight">
                  {editingImpact ? "Edit" : "Create New"} <span className="text-blue-600">Impact</span>
                </h2>
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mt-1">Fill in the details below</p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 hover:bg-slate-100 transition-all active:scale-90">
                <FaTimes />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-10 overflow-y-auto flex-1 space-y-10 custom-scrollbar">
              {/* Image Upload */}
              <div className="relative group cursor-pointer" onClick={() => document.getElementById('impact-image').click()}>
                <div className="aspect-[21/9] rounded-[2rem] bg-slate-50 border-2 border-dashed border-slate-200 overflow-hidden flex flex-col items-center justify-center group-hover:bg-slate-100 group-hover:border-blue-400 transition-all">
                  {formData.imagePreview ? (
                    <img src={formData.imagePreview} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="Preview" />
                  ) : (
                    <>
                      ...
                      <p className="text-sm font-black text-slate-400 uppercase tracking-widest">Click to upload cover image</p>
                    </>
                  )}
                  <div className="absolute inset-0 bg-blue-600/20 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center backdrop-blur-[2px]">
                    <div className="px-6 py-3 bg-white rounded-xl shadow-xl font-black text-blue-600 text-xs tracking-widest">CHANGE IMAGE</div>
                  </div>
                </div>
                <input type="file" id="impact-image" className="hidden" accept="image/*" onChange={handleImageChange} />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <InputField label="Dates / Timeline" name="dates" value={formData.dates} onChange={handleInputChange} required placeholder="e.g., March 01, 2022 - April 30, 2022" />
                <InputField label="Location (Optional)" name="location" value={formData.location} onChange={handleInputChange} placeholder="e.g., National or Multi-City" />
              </div>

              <div className="grid md:grid-cols-2 gap-8">
                <InputField label="Platform / Venue" name="platform" value={formData.platform} onChange={handleInputChange} placeholder="e.g., Zoom, Physical, etc." />
                <div className="flex items-center gap-4 ml-4">
                  ...
                  />
                  <label htmlFor="isLarge" className="text-xs font-black text-slate-500 uppercase tracking-widest">Large Card Layout</label>
                </div>
              </div>

              <TextAreaField label="Short Description" name="description" value={formData.description} rows={2} onChange={handleInputChange} required placeholder="A brief summary for the impact..." />
              <TextAreaField label="Full Detailed Story" name="details" value={formData.details} rows={6} onChange={handleInputChange} placeholder="Enter the complete impact story here..." />
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-sm font-black text-primary uppercase tracking-[0.2em]">Impact Outcomes (Metrics)</h3>
                </div>
                <div className="grid md:grid-cols-3 gap-8 p-8 bg-slate-50 rounded-[2rem] border border-slate-100">
                  <InputField label="Participants" name="participants" type="number" value={formData.participants} onChange={handleInputChange} />
                  <InputField label="Projects" name="projects" type="number" value={formData.projects} onChange={handleInputChange} />
                  <InputField label="Impact Note" name="impactNote" value={formData.impactNote} onChange={handleInputChange} placeholder="e.g., 80% Success Rate" />
                </div>
              </div>
              ...
              <div className="flex justify-end gap-4 pb-4">
                ...
                >
                  {submitting ? "SAVING..." : (editingImpact ? "UPDATE IMPACT" : "SAVE IMPACT")}
                </button>
              </div>
            </form>
          </div>
      ...
      {viewingImpact && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-md" onClick={() => setViewingImpact(null)} />
            ...
            <div className="p-10 overflow-y-auto custom-scrollbar space-y-8">
              <div className="grid grid-cols-2 gap-6 pb-8 border-b border-slate-100">
                <div className="space-y-1">
                  ...
                  <p className="font-bold text-slate-700">{viewingImpact.dates}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">LOCATION</p>
                  <p className="font-bold text-slate-700">{viewingImpact.location || "National"}</p>
                </div>
              </div>

              <div className="space-y-4 text-slate-600 leading-relaxed font-medium">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Story Highlights</h3>
              ...
              <div className="space-y-4">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Verified Metrics</h3>
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50 text-center">
                    <p className="text-[10px] font-black text-blue-400 uppercase mb-1">Participants</p>
                    <p className="text-xl font-black text-blue-600">{viewingImpact.metrics?.participants || 0}</p>
                  </div>
                  <div className="bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100/50 text-center">
                    <p className="text-[10px] font-black text-emerald-400 uppercase mb-1">Projects</p>
                    <p className="text-xl font-black text-emerald-600">{viewingImpact.metrics?.projects || 0}</p>
                  </div>
                  <div className="bg-amber-50/50 p-4 rounded-2xl border border-amber-100/50 text-center">
                    <p className="text-[10px] font-black text-amber-500 uppercase mb-1">Impact Score</p>
                    <p className="text-xl font-black text-amber-600 font-mono text-xs">{viewingImpact.metrics?.impact || "N/A"}</p>
                  </div>
                </div>
              </div>
