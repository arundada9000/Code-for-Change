...
function Certificate() {
  ...
  const fetchCertificates = async () => {
    try {
      ...
      const { data } = await API.get(`/certificates/ledger?${params.toString()}`);
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    try {
      ...
      setCertificates(certificates.map(c => c._id === id ? { ...c, status: newStatus } : c));
      setOpenMenuId(null);
  ...
  const generatePDF = async (cert) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(100, 116, 139);
    doc.text(`REGD NO.: ${cert.regdNo || "64498-066-067"}`, 270, 22, { align: "right" });
    doc.text(`CERTIFICATE NO.: ${cert.certificateId}`, 270, 27, { align: "right" });
    doc.text(`ISSUED DATE: ${formatDate(cert.issueDate)}`, 270, 32, { align: "right" });
        ...
        switch(type) {
          ...
          case "Workshop":
            return {
              ...
              primaryDetail: cert.hours ? `${cert.hours} of professional` : "professional"
            };
          case "Internship":
            return {
              header: "Experience",
    ...
    doc.setFontSize(36);
    doc.setFont("helvetica", "bolditalic");
    doc.text(cert.recipientName.toUpperCase(), pageWidth / 2, 110, { align: "center" });
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text(`${config.primaryDetail} ${cert.courseName}`, pageWidth / 2, 134, { align: "center" });
    ...
    try {
      if (qrCodeDataURL) {
        doc.addImage(qrCodeDataURL, "PNG", footerX + padding, footerY + padding, qrSize, qrSize);
      }
    } catch (e) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
           <div className="hidden md:flex gap-2 mr-4">
            <div className="bg-emerald-50 px-4 py-2 rounded-xl text-center border border-emerald-100">
               ...
               <p className="text-lg font-black text-emerald-700 leading-none">{certificates.filter(c => c.status === 'Valid').length}</p>
            </div>
            <div className="bg-slate-50 px-4 py-2 rounded-xl text-center border border-slate-100">
               ...
               <p className="text-lg font-black text-slate-700 leading-none">{certificates.length}</p>
            </div>
          </div>
           {hasPermission('certificate_issue') && (
             ...
             >
               <FaPlus className="text-lg" /> <span className="hidden sm:inline">Issue Certificate</span><span className="sm:hidden">Issue New</span>
             </button>
           )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden min-h-[500px] flex flex-col">
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="min-w-full w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr><td colSpan="5" className="p-20 text-center text-slate-400 font-black uppercase tracking-widest animate-pulse">Loading certificates...</td></tr>
              ) : certificates.length === 0 ? (
                <tr><td colSpan="5" className="p-20 text-center text-slate-400 font-black uppercase tracking-widest">No matching credentials found</td></tr>
              ) : certificates.map((cert, index) => (
                <tr key={cert._id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'} hover:bg-slate-100/50 transition-all group`}>
                  <td className="px-8 py-6">
                     <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center p-2">
                           <QRCode value={`${window.location.origin}/certificate-verification/${cert.certificateId}`} size={24} />
                        </div>
                  ...
                  <td className="px-8 py-6">
                    <div>
                      <div className="font-black text-slate-900 group-hover:text-emerald-600 transition-colors uppercase tracking-tight">{cert.recipientName}</div>
                      <div className="text-[10px] font-bold text-slate-400 truncate max-w-[150px]">{cert.recipientEmail} • {cert.province || "N/A"}</div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6">
                    <div className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.1em] ${
                      cert.status === 'Valid' ? 'text-emerald-500' : 'text-rose-500'
                    }`}>
                  ...
                  <td className="px-8 py-6 text-right relative">
                    <button 
                      onClick={() => setOpenMenuId(openMenuId === cert._id ? null : cert._id)}
                    ...
                    {openMenuId === cert._id && (
                      <div className="absolute right-20 top-6 w-52 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 py-2 animate-in fade-in zoom-in duration-200">
                         ...
                         >
                           <BsDownload className="text-emerald-500" /> Export PDF
                         </button>
                         <button 
                         ...
                         >
                          ...
                          {hasPermission('certificate_update') && (
                            <>
                              <div className="px-5 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest">Change Status</div>
                              <div className="flex gap-1 px-4 pb-2">
                                 {['Valid', 'Revoked', 'Expired'].map(status => (
                                   <button
                                     key={status}
                                     onClick={() => handleUpdateStatus(cert._id, status)}
        ...
        <div className="md:hidden p-4 space-y-4">
           {loading ? (
              <div className="p-10 text-center text-slate-400 font-black uppercase tracking-widest animate-pulse">Loading certificates...</div>
            ) : certificates.length === 0 ? (
              <div className="p-10 text-center text-slate-400 font-black uppercase tracking-widest">No matching credentials found</div>
            ) : certificates.map(cert => (
              <div key={cert._id} className="bg-slate-50 rounded-2xl p-5 space-y-4 border border-slate-100 shadow-sm relative">
                 <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                       <div className="w-12 h-12 bg-white rounded-xl shadow-sm border border-slate-100 flex items-center justify-center p-2">
                          <QRCode value={`${window.location.origin}/certificate-verification/${cert.certificateId}`} size={32} />
                       </div>
                       <div>
                          <p className="font-mono text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded w-fit mb-1">{cert.certificateId}</p>
                          <h3 className="font-black text-slate-900 uppercase tracking-tight leading-none text-sm">{cert.recipientName}</h3>
                       </div>
                    </div>
                    <button 
                      onClick={() => setOpenMenuId(openMenuId === cert._id ? null : cert._id)}
                 ...
                 {openMenuId === cert._id && (
                    <div className="absolute top-14 right-4 w-52 bg-white rounded-xl shadow-2xl border border-slate-100 z-50 py-2 animate-in fade-in zoom-in">
                       <button onClick={() => generatePDF(cert)} className="w-full px-4 py-2 text-left text-xs font-bold text-slate-600 hover:bg-slate-50">Export PDF</button>
                        <button onClick={() => handlePreview(cert)} className="w-full px-4 py-2 text-left text-xs font-bold text-slate-600 hover:bg-slate-50">Preview</button>
                        <div className="h-[1px] bg-slate-50 my-1"></div>
                        {hasPermission('certificate_delete') && (
                          <button onClick={() => { setCertToDelete(cert); setDeleteModalOpen(true); }} className="w-full px-4 py-2 text-left text-xs font-bold text-rose-500 hover:bg-rose-50">Delete</button>
                        )}
                     </div>
                 )}
                 ...
                 <div className="bg-white p-3 rounded-xl border border-slate-100">
                    ...
                    <p className="text-xs font-bold text-slate-700 uppercase mt-0.5 leading-tight">{cert.courseName}</p>
                 </div>
                 
                 <div className="flex justify-between items-center pt-1">
                    <span className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${
                      cert.status === 'Valid' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'
                    }`}>
                       <FaCheckCircle /> {cert.status}
                    </span>
                    <span className="text-[9px] font-bold text-slate-400">{cert.province || "N/A"} • {new Date(cert.issueDate).toLocaleDateString()}</span>
                  </div>
              </div>
            ))}
        </div>
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-xl p-10 shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="flex justify-between items-center mb-8 border-b border-slate-50 pb-6">
              <div>
                ...
                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mt-1">Official Certification Form</p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="w-12 h-12 flex items-center justify-center bg-slate-50 rounded-2xl text-slate-300 hover:bg-rose-50 hover:text-rose-500 transition-all">
                <FaTimes size={20} />
              </button>
            </div>
            ...
            {showPreview ? (
              ...
              <form onSubmit={handleIssue} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Recipient Full Name</label>
                  <input 
                    name="recipientName" 
                    required 
                ...
                <div className="md:col-span-2">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Recipient Email</label>
                  <input 
                    name="recipientEmail" 
                    type="email" 
                    required 
                ...
                <div className="md:col-span-2">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Program / Course Name</label>
                  <input 
                    name="courseName" 
                    required 
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Certificate Type</label>
                  <select 
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Province</label>
                  <select 
                    name="province" 
                    className="w-full p-4 bg-slate-50 border border-transparent rounded-2xl outline-none focus:border-emerald-500/30 focus:bg-white font-bold transition-all mt-1"
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Custom ID (Optional)</label>
                  <input 
                    name="certificateId" 
                ...
                {(['Training', 'Bootcamp', 'Workshop', 'Internship'].includes(formData.certificateType)) && (
                  <>
                    <div className={formData.certificateType === 'Internship' ? 'md:col-span-2' : ''}>
                ...
                {(['Training', 'Bootcamp', 'Workshop'].includes(formData.certificateType)) && (
                  <div>
                    <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Accredited Hours</label>
                    <input 
                      name="hours" 
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Grade / Result</label>
                  <input 
                    name="grade" 
                    placeholder={['Event', 'Hackathon'].includes(formData.certificateType) ? "e.g. Winner / Participation" : "e.g. Grade A"}
                    className="w-full p-4 bg-slate-50 border border-transparent rounded-2xl outline-none focus:border-emerald-500/30 focus:bg-white font-bold transition-all mt-1" 
        ...
        <div className="fixed inset-0 bg-slate-900/98 backdrop-blur-2xl z-[200] flex flex-col items-center justify-center p-4 md:p-12 overflow-hidden">
           ...
           <div className="w-full max-w-7xl flex flex-col md:flex-row items-center justify-between mb-4 md:mb-8 animate-in slide-in-from-top-4 duration-500 gap-4">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    <h3 className="text-white font-black uppercase italic tracking-tighter text-lg md:text-xl leading-none">Certificate Preview</h3>
                    <p className="text-emerald-400 text-[8px] md:text-[9px] font-black uppercase tracking-[0.3em] mt-1">Official Record: {previewData?.certificateId}</p>
                 </div>
              </div>
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                 ...
                 >
                    <FaExternalLinkAlt /> <span className="hidden md:inline">Open </span>Print<span className="hidden md:inline"> Dialogue</span>
                 </button>
                 <button 
           ...
           <div className="flex-1 w-full max-w-6xl flex items-center justify-center animate-in zoom-in-95 duration-500 overflow-auto custom-scrollbar p-2">
              <div className="w-full h-fit flex items-center justify-center p-0 md:p-4">
                ...
                <div className="w-full max-w-5xl shadow-[0_0_100px_rgba(0,0,0,0.8)] border-[1px] border-white/10 rounded-sm scale-50 md:scale-100 origin-center md:origin-top transition-transform duration-300">
                   <CertificatePreview data={{ ...previewData, tokenHash: previewData?.tokenHash || 'preview-only' }} />
                </div>
              </div>
           </div>
           ...
           <div className="mt-4 md:mt-8 text-center animate-in slide-in-from-bottom-4 duration-500 hidden md:block">
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.5em]">Global Accreditation • ISO 9001:2015 Verified</p>
           </div>
        </div>
      )}
    </div>
