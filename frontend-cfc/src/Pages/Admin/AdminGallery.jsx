...
import { FaPlus, FaSearch, FaRegTrashAlt, FaTimes, FaCloudUploadAlt } from "react-icons/fa";
...
function AdminGallery() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setItems(items.filter(item => (item._id || item.id) !== (itemToDelete._id || itemToDelete.id)));
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      <div className="flex justify-between items-center px-4">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Gallery Management</h2>
          <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Manage Public Gallery Images</p>
          <select
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden min-h-[600px]">
        {loading ? (
          <div className="p-10 text-center text-slate-400 font-bold">Loading gallery items...</div>
        ) : items.length === 0 ? (
          <div className="p-10 text-center text-slate-400 font-bold">No gallery items found.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-8">
            {items.map((item) => (
              <div key={item._id || item.id} className="group relative bg-slate-50 rounded-2xl overflow-hidden border border-slate-100 transition-all hover:shadow-xl">
                <img src={item.imageUrl} className="w-full h-48 object-cover transition-transform duration-500 group-hover:scale-110" alt={item.title} />
                <div className="p-4">
                  <h3 className="font-bold text-slate-900 line-clamp-1">{item.title}</h3>
                  <p className="text-[10px] text-emerald-600 font-black uppercase tracking-widest mt-1">{item.category} • {item.province || "National"}</p>
                </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-6 overflow-y-auto">
              <InputField label="Image Title" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} required placeholder="e.g. Workshop Session" />
              ...
              <div
                ...
                className={`relative border-2 border-dashed rounded-[2rem] flex flex-col items-center justify-center transition-all cursor-pointer overflow-hidden min-h-[250px] ${isDragging ? "border-emerald-500 bg-emerald-50" : "border-slate-200 bg-slate-50 hover:border-emerald-500"
                  }`}
              >
                ...
                {formData.imagePreview ? (
                  ...
                  <div className="text-center p-6 text-slate-400">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest">Drop gallery image here</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                />
                <label htmlFor="isFeaturedG" className="text-xs font-black text-slate-700 uppercase tracking-widest cursor-pointer">
                  Featured Image
                </label>
              </div>
