...
import { 
  FaPlus, FaSearch, FaBriefcase, FaBuilding, FaMapMarkerAlt, FaEdit, FaTrash, FaTimes, 
  FaImage, FaCloudUploadAlt, FaExternalLinkAlt, FaMoneyBillWave, FaClock, FaLink, 
  FaFileCsv, FaFilePdf, FaFileWord, FaDownload, FaDownload as FaImport, FaRegCalendarAlt, FaChevronRight, FaUserTie
} from "react-icons/fa";
...
function AdminInternships() {
  ...
  }, [searchTerm, filterType, filterStatus, filterDeadlineStart, filterDeadlineEnd, filterProvince]);
  ...
  const handleOpenModal = (internship = null) => {
    if (internship) {
      ...
      setFormData({ 
        title: "", companyName: "", location: "", type: "Internship", category: "", 
        description: "", requirements: [], responsibilities: [], salaryRange: "",
        applicationDeadline: "", applyLink: "", status: "Draft", postedBy: "Code for Change",
        province: "",
  ...
  const handleDelete = async () => {
    ...
    try {
      await API.delete(`/internships/${internshipToDelete._id || internshipToDelete.id}`);
      setInternships(internships.filter(i => (i._id || i.id) !== (internshipToDelete._id || internshipToDelete.id)));
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(formData).forEach(key => {
        ...
        } else if (formData[key] !== null && key !== 'logoPreview' && key !== 'logoFile') {
    ...
    try {
      if (editingInternship) {
        await API.put(`/internships/${editingInternship._id || editingInternship.id}`, data, {
  ...
  const exportToCSV = () => {
    const csv = Papa.unparse(internships.map(i => ({
      Title: i.title,
      Company: i.companyName,
      Location: i.location,
      Type: i.type,
      Category: i.category,
      Province: i.province || "N/A",
      Status: i.status,
      Deadline: i.applicationDeadline ? new Date(i.applicationDeadline).toLocaleDateString() : "N/A"
    })));
  ...
  const exportToPDF = () => {
    ...
    const tableColumn = ["Company", "Position", "Province", "Type", "Status", "Deadline"];
    const tableRows = internships.map(i => [
      ...
      i.type,
      i.status,
      i.applicationDeadline ? new Date(i.applicationDeadline).toLocaleDateString() : "N/A"
    ]);
  ...
  const exportToWord = () => {
    ...
    internships.forEach((i, index) => {
      ...
      content += `   Type: ${i.type}\n`;
      content += `   Province: ${i.province || "N/A"}\n`;
        ...
        try {
          ...
          for (const row of results.data) {
            const payload = {
              ...
              description: row.Description || row.description || "Bulk imported vacancy",
              applyLink: row.ApplyLink || row.applyLink || "https://codeforchange.org.np",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Vacancy Management</h2>
          <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Jobs & Internships Registry</p>
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="grid grid-cols-2 md:flex gap-3 md:gap-4 w-full md:w-auto">
            ...
            >
              ...
              {['Full-time', 'Internship', 'Contract', 'Part-time', 'Remote'].map(t => (
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="flex items-center gap-2 w-full md:w-auto">
                 <label className="text-xs font-bold text-slate-400 uppercase tracking-wider min-w-[80px]">Deadline From:</label>
                 <input 
                    type="date" 
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden min-h-[600px]">
        <div className="hidden md:block overflow-x-auto">
          <table className="min-w-full w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr><td colSpan="4" className="p-10 text-center text-slate-400">Loading vacancies...</td></tr>
              ) : internships.length === 0 ? (
                <tr><td colSpan="4" className="p-10 text-center text-slate-400">No vacancies found.</td></tr>
              ) : (
                internships.map((item, index) => (
                  <tr key={item._id || item.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-slate-50'} hover:bg-slate-100/50 transition-all`}>
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-100 italic font-black text-slate-400 text-xl shadow-sm">
                          {item.companyLogo ? (
                            <img src={item.companyLogo} alt={item.companyName} className="w-full h-full object-cover" />
                          ) : item.companyName[0]}
                        </div>
                        <div>
                          <div className="font-black text-slate-900 leading-tight text-base mb-1 hover:text-emerald-600 transition-colors cursor-pointer" onClick={() => handleOpenDetail(item)}>{item.title}</div>
                    ...
                    <td className="px-8 py-6">
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2 text-sm font-bold text-slate-700">
                             <FaMapMarkerAlt className="text-rose-400" size={12} /> {item.province || "National"} • {item.location}
                        </div>
                        <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest flex items-center gap-1">
                            <FaClock className="text-slate-300" /> {item.type} • {item.category}
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                        <div className="flex items-center gap-2 text-sm font-bold text-slate-700">
                            <FaRegCalendarAlt className="text-emerald-500" size={12} />
                            {item.applicationDeadline ? new Date(item.applicationDeadline).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}
                        </div>
                        <span className={`inline-flex mt-2 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                            item.status === 'Open' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                        }`}>
                            {item.status}
                        </span>
                    </td>
                    <td className="px-8 py-6 text-right relative">
                      ...
                      {openMenuId === item._id && (
                        <div className="absolute right-20 top-6 w-52 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 py-2 animate-in fade-in slide-in-from-right-5 duration-200">
                           <button 
                            onClick={() => { handleOpenDetail(item); setOpenMenuId(null); }}
                          ...
                          >
                           ...
                           {hasPermission('internship_update') && (
                             <>
                              ...
                              <button 
                                onClick={() => { handleOpenModal(item); setOpenMenuId(null); }}
                                className="w-full px-5 py-3 text-left flex items-center gap-3 text-xs font-black text-slate-700 hover:bg-slate-50 transition-all uppercase tracking-widest"
                              >
                                <BsPencil className="text-amber-500" /> Edit Vacancy
                              </button>
                             </>
                           )}
        ...
        <div className="md:hidden p-4 space-y-4">
          {loading ? (
             <div className="p-10 text-center text-slate-400">Loading vacancies...</div>
          ) : internships.length === 0 ? (
             <div className="p-10 text-center text-slate-400">No vacancies found.</div>
          ) : (
            internships.map((item) => (
              <div key={item._id || item.id} className="bg-slate-50 rounded-2xl p-5 space-y-4 border border-slate-100 shadow-sm relative">
                 <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          <h3 className="font-black text-slate-900 text-sm leading-tight line-clamp-2">{item.title}</h3>
                          <div className="flex items-center gap-1 mt-1">
                             <FaBuilding size={10} className="text-emerald-500" />
                             <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{item.companyName}</span>
                          </div>
                       </div>
                    </div>
                    <button 
                 ...
                 {openMenuId === item._id && (
                    <div className="absolute top-14 right-4 w-52 bg-white rounded-xl shadow-2xl border border-slate-100 z-50 py-2 animate-in fade-in zoom-in">
                       <button onClick={() => { handleOpenDetail(item); setOpenMenuId(null); }} className="w-full px-4 py-2 text-left text-xs font-bold text-slate-600 hover:bg-slate-50">View Detail</button>
                       {hasPermission('internship_update') && (
                         <button onClick={() => { handleOpenModal(item); setOpenMenuId(null); }} className="w-full px-4 py-2 text-left text-xs font-bold text-amber-600 hover:bg-amber-50">Edit Vacancy</button>
                       )}
                       <div className="h-[1px] bg-slate-50 my-1"></div>
                       {hasPermission('internship_delete') && (
                         <button onClick={() => { setInternshipToDelete(item); setDeleteModalOpen(true); }} className="w-full px-4 py-2 text-left text-xs font-bold text-rose-500 hover:bg-rose-50">Delete</button>
                       )}
                    </div>
                 )}
                 ...
                 <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white p-3 rounded-xl border border-slate-100">
                       ...
                       <p className="text-xs font-bold text-slate-700 uppercase mt-0.5">{item.type}</p>
                    </div>
                    <div className="bg-white p-3 rounded-xl border border-slate-100">
                       <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Deadline</p>
                       <p className="text-xs font-bold text-slate-700 uppercase mt-0.5 truncate">
                          {item.applicationDeadline ? new Date(item.applicationDeadline).toLocaleDateString() : 'N/A'}
                       </p>
                    </div>
                 </div>
                 ...
                 <div className="flex justify-between items-center pt-1">
                    <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[9px] font-black uppercase tracking-widest ${
                        item.status === 'Open' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                    }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${item.status === 'Open' ? 'bg-emerald-500' : 'bg-amber-500'}`}></span> {item.status}
                    </span>
                    <div className="flex items-center gap-1 text-[9px] font-bold text-slate-400">
                       <FaMapMarkerAlt size={10} /> {item.province || "N/A"} • {item.location}
                    </div>
                 </div>
              </div>
            ))
                    ...
                    <div className="flex items-center gap-6">
                        <div className="w-16 h-16 rounded-2xl bg-white shadow-sm flex items-center justify-center p-2 border border-slate-100 overflow-hidden">
                             {selectedInternship.companyLogo ? (
                                <img src={selectedInternship.companyLogo} className="w-full h-full object-contain" alt="Logo" />
                             ) : <FaBuilding className="text-slate-200" size={32} />}
                        </div>
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 tracking-tight">{selectedInternship.title}</h3>
                            <div className="flex items-center gap-3 mt-1">
                                <span className="text-xs font-black text-emerald-600 uppercase tracking-widest">{selectedInternship.companyName}</span>
                                <span className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.2em] border-l pl-3">{selectedInternship.type}</span>
                            </div>
                        </div>
                    </div>
                    <button onClick={() => setIsDetailModalOpen(false)} className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white text-slate-400 hover:text-rose-500 hover:shadow-lg transition-all border border-slate-100 group">
                ...
                <div className="flex-1 overflow-y-auto p-10 space-y-12 scrollbar-hide">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center gap-5 group hover:bg-white hover:shadow-xl hover:shadow-slate-100 transition-all">
                             ...
                             <div>
                                ...
                                <p className="text-sm font-bold text-slate-800">{selectedInternship.province || "N/A"} • {selectedInternship.location}</p>
                             </div>
                        </div>
                        <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center gap-5 group hover:bg-white hover:shadow-xl hover:shadow-slate-100 transition-all">
                             ...
                             <div>
                                ...
                                <p className="text-sm font-bold text-slate-800">{selectedInternship.salaryRange || 'Not Specified'}</p>
                             </div>
                        </div>
                        <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center gap-5 group hover:bg-white hover:shadow-xl hover:shadow-slate-100 transition-all">
                             ...
                             <div>
                                ...
                                <p className="text-sm font-bold text-slate-800">
                                    {selectedInternship.applicationDeadline ? new Date(selectedInternship.applicationDeadline).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : 'No Deadline Set'}
                                </p>
                             </div>
                        </div>
                    </div>
                    ...
                    <section className="space-y-4">
                        <div className="flex items-center gap-3">
                             ...
                             <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">Role Description</h4>
                        </div>
                        <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                             <p className="text-slate-600 leading-relaxed font-medium whitespace-pre-wrap">{selectedInternship.description}</p>
                        </div>
                    </section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                         <section className="space-y-6">
                            <div className="flex items-center gap-3">
                                ...
                                <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">Key Requirements</h4>
                            </div>
                            <div className="space-y-3">
                                {selectedInternship.requirements?.length > 0 ? selectedInternship.requirements.map((req, i) => (
                                    <div key={i} className="flex items-start gap-4 p-4 bg-blue-50/30 rounded-2xl border border-blue-100/30">
                                        <FaChevronRight className="mt-1 text-blue-400 flex-shrink-0" size={10} />
                                        <span className="text-sm text-slate-700 font-medium">{req}</span>
                                    </div>
                                )) : <p className="text-slate-400 italic text-sm">No specific requirements listed.</p>}
                            </div>
                         </section>
                         ...
                         <section className="space-y-6">
                            <div className="flex items-center gap-3">
                                ...
                                <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">Core Responsibilities</h4>
                            </div>
                            <div className="space-y-3">
                                {selectedInternship.responsibilities?.length > 0 ? selectedInternship.responsibilities.map((res, i) => (
                                    <div key={i} className="flex items-start gap-4 p-4 bg-purple-50/30 rounded-2xl border border-purple-100/30">
                                        <FaChevronRight className="mt-1 text-purple-400 flex-shrink-0" size={10} />
                                        <span className="text-sm text-slate-700 font-medium">{res}</span>
                                    </div>
                                )) : <p className="text-slate-400 italic text-sm">No specific responsibilities listed.</p>}
                            </div>
                         </section>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 pt-8">
                         ...
                         >
                            <FaUserTie /> Posted By: {selectedInternship.postedBy || 'CFC Admin'}
                         </button>
                    </div>
                </div>
            </div>
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-6xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="flex justify-between items-center px-10 py-8 border-b border-slate-50 flex-shrink-0">
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Registry Management</p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-50 text-slate-400 hover:text-emerald-600 transition-all">
                <FaTimes />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-8 overflow-y-auto scrollbar-hide">
              <div className="space-y-6">
                <h4 className="text-xs font-black text-emerald-500 uppercase tracking-widest">Base Information</h4>
                <div className="grid grid-cols-2 gap-4">
                  <InputField label="Position Title" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} required placeholder="e.g., Full Stack Developer" />
                  <InputField label="Company Name" value={formData.companyName} onChange={(e) => setFormData({...formData, companyName: e.target.value})} required placeholder="e.g., Tesla" />
                </div>
                <div className="grid grid-cols-3 gap-4">
                     <InputField label="Location" value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} required placeholder="e.g., Kathmandu / Remote" />
                     <div className="space-y-1.5">
                       <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-4">Province</label>
                       <select className="w-full px-6 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-sm font-medium hover:bg-white focus:ring-4 focus:ring-emerald-500/5 transition-all" value={formData.province} onChange={(e) => setFormData({...formData, province: e.target.value})}>
                         <option value="">Select Province</option>
                         {PROVINCES.map(p => <option key={p} value={p}>{p}</option>)}
                       </select>
                     </div>
                     <InputField label="Category" value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} required placeholder="e.g., Tech / Design" />
                 </div>
                 <div className="grid grid-cols-2 gap-4">
                     <InputField label="Salary Range" value={formData.salaryRange} onChange={(e) => setFormData({...formData, salaryRange: e.target.value})} placeholder="e.g., Negotiable / 20k - 30k" />
                 </div>
              </div>

              <div className="space-y-4">
                <h4 className="text-xs font-black text-emerald-500 uppercase tracking-widest">Job Specifications</h4>
                <TextAreaField label="Role Description" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} required placeholder="Detailed overview of the role..." />
                
                <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Requirements</label>
                        <div className="flex gap-2">
                             <input type="text" id="req-input" placeholder="Add requirement..." className="flex-1 px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:bg-white transition-all" onKeyPress={(e) => { if (e.key === 'Enter') { e.preventDefault(); addToList('requirements', e.target.value); e.target.value = ''; }}} />
                             <button type="button" onClick={() => { const input = document.getElementById('req-input'); addToList('requirements', input.value); input.value = ''; }} className="bg-emerald-500 text-white px-6 rounded-2xl font-black text-lg shadow-lg shadow-emerald-100 hover:bg-emerald-600 transition-all active:scale-95">+</button>
                        </div>
                        <div className="space-y-2 max-h-[150px] overflow-y-auto pr-2 custom-scrollbar">
                            {formData.requirements.map((r, i) => (
                                <div key={i} className="flex justify-between items-center bg-slate-50 p-3 px-4 rounded-xl text-xs font-bold text-slate-600 border border-slate-100">
                                    <span>{r}</span>
                                    <button type="button" onClick={() => removeFromList('requirements', i)} className="text-rose-400 hover:text-rose-600 transition-colors"><FaTimes size={12} /></button>
                                </div>
                            ))}
                        </div>
                    </div>
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Responsibilities</label>
                        <div className="flex gap-2">
                             <input type="text" id="res-input" placeholder="Add responsibility..." className="flex-1 px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:bg-white transition-all" onKeyPress={(e) => { if (e.key === 'Enter') { e.preventDefault(); addToList('responsibilities', e.target.value); e.target.value = ''; }}} />
                             <button type="button" onClick={() => { const input = document.getElementById('res-input'); addToList('responsibilities', input.value); input.value = ''; }} className="bg-emerald-500 text-white px-6 rounded-2xl font-black text-lg shadow-lg shadow-emerald-100 hover:bg-emerald-600 transition-all active:scale-95">+</button>
                        </div>
                        <div className="space-y-2 max-h-[150px] overflow-y-auto pr-2 custom-scrollbar">
                            {formData.responsibilities.map((r, i) => (
                                <div key={i} className="flex justify-between items-center bg-slate-50 p-3 px-4 rounded-xl text-xs font-bold text-slate-600 border border-slate-100">
                                    <span>{r}</span>
                                    <button type="button" onClick={() => removeFromList('responsibilities', i)} className="text-rose-400 hover:text-rose-600 transition-colors"><FaTimes size={12} /></button>
                                </div>
                            ))}
                        </div>
                    </div>
              ...
              <div className="grid grid-cols-4 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <select className="w-full px-6 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-sm font-medium hover:bg-white focus:ring-4 focus:ring-emerald-500/5 transition-all" value={formData.type} onChange={(e) => setFormData({...formData, type: e.target.value})}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select className="w-full px-6 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-sm font-medium hover:bg-white focus:ring-4 focus:ring-emerald-500/5 transition-all" value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value})}>
                  ...
                  <div className="col-span-2">
                    <InputField label="Application Deadline" type="date" value={formData.applicationDeadline} onChange={(e) => setFormData({...formData, applicationDeadline: e.target.value})} />
                  </div>
              </div>

              <InputField label="Apply Link" value={formData.applyLink} onChange={(e) => setFormData({...formData, applyLink: e.target.value})} required placeholder="https://external-job-portal.com" />

              <div className="space-y-3">
                <h4 className="text-xs font-black text-emerald-500 uppercase tracking-widest">Company Logo</h4>
                <div
                ...
                >
                  ...
                  {formData.logoPreview ? (
                    <>
                      <img src={formData.logoPreview} className="max-h-full max-w-full object-contain p-4" alt="Preview" />
                    ...
                    <div className="text-center p-6 text-slate-400">
                      ...
                      <p className="text-sm font-bold text-slate-600">Click or Drag Company Logo</p>
                      <p className="text-[10px] uppercase font-black tracking-widest mt-1 opacity-50">PNG, JPG, SVG are supported</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-4 pt-6">
                ...
                <button type="submit" disabled={submitting} className="flex-1 py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-emerald-700 shadow-xl shadow-emerald-100 disabled:opacity-70 transition-all active:scale-[0.98]">
                  {submitting ? "Processing..." : (editingInternship ? "Save Changes" : "Post Vacancy")}
                </button>
              </div>
            </form>
          </div>
