...
const CertificatePreview = ({ data }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width < 640) setQrSize(25);      // mobile
      else if (width < 768) setQrSize(35); // small tablet
      else if (width < 1024) setQrSize(40); // tablet
  ...
  return (
    <div className="w-full aspect-4/3 bg-white border-[4px] sm:border-[6px] md:border-[8px] lg:border-[12px] border-slate-900 p-2 sm:p-4 md:p-6 lg:p-10 shadow-2xl relative overflow-hidden font-sans text-slate-900 select-none flex flex-col justify-between">
      ...
      {/* Header Section */}
       <div className="flex justify-between items-start relative z-10 shrink-0">         <div className="flex items-center gap-4">           <img src={logo} alt="Logo" className="w-10 sm:w-14 md:w-20" />           <div className="border-l-2 border-slate-200 pl-2">             <h1 className="text-[12px] sm:text-lg md:text-xl font-bold -md:font-black tracking-tight text-slate-800">               CODE FOR CHANGE             </h1>             <p className="text-[8px] font-bold text-slate-500 uppercase leading-none">               Nepal • Since 2024             </p>           </div>         </div>         <div className="text-right text-[8px] font-bold uppercase text-slate-400 leading-relaxed">           <p className="flex justify-end gap-1">             REGD NO.: <span className="text-slate-900">{regdNo}</span>           </p>           <p className="flex justify-end gap-1">             CERTIFICATE NO.:{" "}             <span className="text-slate-900">{certificateId}</span>           </p>           <p className="flex justify-end gap-1">             ISSUED DATE:{" "}             <span className="text-slate-900">{formatDate(issueDate)}</span>           </p>         </div>       </div>
      {/* Main Body */}
      <div className="flex-1 flex flex-col justify-center items-center text-center relative z-10 px-4 sm:px-6 md:px-8 py-2 sm:py-4 md:py-6">
        ...
        <div className="sm:mb-4 md:mb-6">
          <h2 className="text-base sm:text-2xl md:text-3xl lg:text-5xl font-black text-slate-900 tracking-tighter leading-none">
            {config.header}
          </h2>
        ...
        <div className="md:mb-4 w-full">
          <h4 className={`${getRecipientFontSize(recipientName)} font-semibold italic text-slate-900 tracking-tighter uppercase break-words leading-tight`}>
            {recipientName}
          </h4>
        ...
        <div className="w-full max-w-3xl">
          <div className="inline-flex flex-wrap items-center justify-center gap-2 sm:gap-3 border-b md:border-b-2 border-slate-100 pb-1 sm:pb-2">
            ...
            <span className={`${getCourseFontSize(courseName)} font-bold md:font-black text-slate-900 italic tracking-tight break-words`}>
              {courseName}
            </span>
          </div>
      ...
      <div className="flex justify-between items-end sm:items-end gap-4 sm:gap-0 md:mt-4">
        ...
        <div className="text-center flex flex-col items-center">
          <div className="w-6 h-6 sm:w-14 sm:h-14 rounded-full border-3 sm:border-4 border-slate-100 flex flex-col items-center justify-center text-slate-200 mb-1 relative">
            ...
            <p className="relative text-[2px] sm:text-[5px] font-black leading-none">CERTIFIED</p>
            <p className="relative text-[4px] sm:text-xs font-black leading-none my-0.5 text-slate-300">ISO</p>
            <p className="relative text-[2px] sm:text-[5px] font-black leading-none">9001:2015</p>
          </div>
