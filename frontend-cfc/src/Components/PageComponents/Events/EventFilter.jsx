...
import { FaSearch, FaFilter, FaCalendarAlt, FaMapMarkerAlt, FaGlobe, FaStar, FaSortAmountDown } from "react-icons/fa";

const REGIONS = ["Kathmandu", "Pokhara", "Rupandehi", "Dang", "Birgunj", "Farwest", "Koshi", "Chitwan", "LB Karnali"];
const EVENT_TYPES = ["hackathon", "workshop", "webinar", "conference", "social_impact"];
...
const EventFilter = ({ onFilterChange }) => {
  ...
  const handleReset = () => {
    const empty = { search: "", type: "", region: "", year: "", sort: "newest", isNational: "" };
    setFilters(empty);
    onFilterChange({});
  };

  const activeCount = [filters.search, filters.type, filters.region, filters.year, filters.isNational]
    .filter(Boolean).length;

  return (
    <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-6 mb-10 transition-all hover:shadow-2xl">
      {/* Top Row */}
      ...
      {/* Filter Grid */}
      <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 transition-all duration-300 ${isExpanded ? 'block' : 'hidden md:grid'}`}>
        {/* Event Type */}
        <div className="space-y-2">
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
            <FaGlobe className="text-blue-400" /> Type
          </label>
          <select
            value={filters.type}
            onChange={(e) => handleChange("type", e.target.value)}
            className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all font-medium text-slate-700 appearance-none cursor-pointer text-sm"
          >
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
            <FaMapMarkerAlt className="text-amber-400" /> Region
          </label>
          <select
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
            <FaStar className="text-amber-400" /> Scope
          </label>
          <select
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
            <FaSortAmountDown className="text-purple-400" /> Sort
          </label>
          <select
            value={filters.sort}
